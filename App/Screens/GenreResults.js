...
const GenreResults = ({route,navigation}) => {
    ...
    const getMovieByGenre = async() => {
        setLoading(true);
        const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${genre}`);
  ...
  return (
    ...
    <Text style={styles.headerContent}>Popular in {route.params.genre.name} <MaterialCommunityIcons name="movie" size={30}/></Text>
    </View>
    {/* base view */}
