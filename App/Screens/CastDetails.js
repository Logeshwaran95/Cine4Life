...
const FavouritesScreen = ({route,navigation}) => {
  ...
  return (
    <View style={{backgroundColor:isDark?colors.dark:colors.light,...styles.container}}>
      ...
      {/* base view */}
      <View style={{margin:5,maxHeight:height/1.13}}>
      <ScrollView>
          ...
          <View style={styles.outline}>
              ...
              {images.length !==0 ? 
              <View>
              <Text style={{marginTop:20,...styles.movieTitle}}>Images</Text>
              <ScrollView horizontal={true}>
              {images.map(image => 
                <Image source={{uri:`https://image.tmdb.org/t/p/w500${image.file_path}`}} style={{width:width-85,height:350,margin:8,borderRadius:20,resizeMode:"cover"}}/>
              )}
               </ScrollView>
               </View> :null}
               ...
               {movies.length !==0 ?
                <View>
              <Text style={{marginTop:30,...styles.movieTitle}}>Movies</Text>
              <ScrollView horizontal={true}>
                {movies.map(movie =>
...
const styles = StyleSheet.create({
  ...
  info : {
      ...
      fontSize:20,
      color:colors.white,
      letterSpacing:1,margin:1,textTransform:"capitalize",lineHeight:24
  },
  rating : {
      marginTop:-5,
      marginRight:38,
